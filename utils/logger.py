...
from constants import MODEL_LOGS_DIR
...
class Logger:
    ...
    def __init__(self, name="CRBot", device_serial=None, log_level="DEBUG", log_to_file=False, log_dir=MODEL_LOGS_DIR):
        """
    ...
    def extra_visibility(self, message):
        """Log a message with high visibility (purple)."""
        if self.log_level <= self.LEVELS["INFO"]:  # Assuming it should be visible at INFO level
            formatted = self._format_message("VISIBILITY", message)
            print(f"{bcolors.PURPLE}{formatted}{bcolors.ENDC}")
            self._write_to_file(formatted)
...
def extra_visibility(message):
    default_logger.extra_visibility(message)